        ...
        <div className="form-page w-350 justify-center">
            ...
            {error && <div style={{ color: 'red' }}>error</div>}
                <form id="new-user-form" action="" method="post" className={`${OlympicSansReg.className} signup-form`} onSubmit={onSubmit}>
                    ...
                    <input type="text" id="surname" name="surname" value={surName} className={inputClass} onBlur={onBlurHandler} onChange={(event)=> setInputSurname(event.target.value)} required/>
                    {onBlurHandler(surName)}
                    
                    <label htmlFor="firstname" className="required">Prénom</label>
                    <input type="text" id="firstname" name="firstname" value={firstName} className={inputClass} onChange={(event)=> setInputFirstname(event.target.value)} required/>
